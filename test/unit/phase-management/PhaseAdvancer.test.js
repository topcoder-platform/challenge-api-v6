...
describe("PhaseAdvancer AI Review phase closure blocking", () => {
  const prisma = getClient();
  const originalFindUnique = prisma.challenge.findUnique;
  ...
  const buildAiReviewPhase = () => ({
    id: "phase-ai-review",
    phaseId: "ai-review-phase-id",
    name: "AI Review",
    ...
    scheduledStartDate: new Date(Date.now() - 60 * 60 * 1000).toISOString(),
    scheduledEndDate: new Date(Date.now() + 23 * 60 * 60 * 1000).toISOString(),
    actualStartDate: new Date(Date.now() - 60 * 60 * 1000).toISOString(),
    actualEndDate: null,
    constraints: [],
  });

  it("blocks closing AI Review phase when there are pending AI decisions", async () => {
    reviewPrisma.getReviewClient = () => ({
      $queryRaw: async () => [{ count: 3 }],
    });

    delete require.cache[phaseAdvancerPath];
    const PhaseAdvancerWithMock = require("../../../src/phase-management/PhaseAdvancer");
    const phaseAdvancer = new PhaseAdvancerWithMock({
      async getPhaseFacts() {
        return {};
      },
    });

    const phases = [buildAiReviewPhase()];
    ...
    expect(result.success).to.be.false;
    expect(result.message).to.contain("pending AI decision");
    expect(result.message).to.contain("3");
    expect(phases[0].isOpen).to.be.true;
    expect(phases[0].actualEndDate).to.be.null;
  });
  ...
  it("allows closing AI Review phase when no pending AI decisions", async () => {
    ...
    expect(result.success).to.be.true;
    expect(phases[0].isOpen).to.be.false;
    expect(phases[0].actualEndDate).to.not.be.null;
  });
});
