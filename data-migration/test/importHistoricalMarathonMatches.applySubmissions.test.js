...
describe("importHistoricalMarathonMatches apply mode submission-history wiring", () => {
  ...
  beforeEach(() => {
    ...
    writeJson(fixtureDir, "long_submission_1.json", "long_submission", [
      ...
      { long_component_state_id: "1003", submission_number: "1", example: "1", submit_time: "1002" },
      { long_component_state_id: "1003", submission_number: "2", example: "1", submit_time: "1003" },
    ]);
  });
  ...
  test("apply-mode materializes latest example-only finalist submission for official participants", async () => {
    const tx = {
      challenge: {
        findMany: jest.fn().mockResolvedValue([]),
        create: jest.fn().mockResolvedValue({ id: "challenge-1" }),
    ...
    const result = await runApplyMode({
      ...
      options: {
        roundIds: ["9892"],
        skippedFilePath: path.join(fixtureDir, "apply-example-only-skipped.json"),
        importSubmissions: true,
        submissionStore,
        dataDir: fixtureDir,
        longComponentStateFile: "long_component_state_1.json",
    ...
    expect(result.records).toEqual([
      expect.objectContaining({
        ...
        submissionReconciliation: {
          legacyNonExampleSubmissions: 2,
          legacyExampleOnlyFinalistSubmissions: 1,
          importedSubmissions: 3,
          alreadyPresentSubmissions: 0,
    ...
    expect(submissionStoreRecords).toEqual(
      new Map([
        ["10010001", { legacySubmissionId: "10010001", memberId: "1", submitter: null }],
        ["10020001", { legacySubmissionId: "10020001", memberId: "2", submitter: null }],
        ["10030002", { legacySubmissionId: "10030002", memberId: "3", submitter: null }],
      ])
    );
  });

  test("apply-mode writes submission archives and URLs when archive generation is configured", async () => {
    const archiveDir = path.join(fixtureDir, "archives");
    ...
    const submissionStore = {
      listExistingSubmissionsByLegacyId: async () => new Map(submissionStoreRecords),
      createSubmission: async ({ legacySubmissionId, memberId, memberHandle }) => {
        submissionStoreRecords.set(legacySubmissionId, {
          legacySubmissionId,
          memberId: String(memberId),
    ...
    const submissionArchiveStore = {
      listExistingSubmissionsByLegacyId: async () =>
        new Map(
          Array.from(submissionStoreRecords.entries()).map(([legacySubmissionId, submission]) => [
            legacySubmissionId,
            {
              legacySubmissionId,
              url: submission.url,
      ...
      updateSubmissionUrl: jest.fn().mockImplementation(async ({ legacySubmissionId, url }) => {
        submissionStoreRecords.get(legacySubmissionId).url = url;
      }),
    };
    ...
    const result = await runApplyMode({
      ...
      options: {
        roundIds: ["9892"],
        skippedFilePath: path.join(fixtureDir, "apply-archive-submission-skipped.json"),
        importSubmissions: true,
        submissionStore,
      ...
      new Map([
        [
          "10010001",
          {
            ...
            memberId: "1",
            submitter: "alpha",
            url: buildSubmissionArchiveUrl({
              archiveFileName: buildSubmissionArchiveFileName({
                challengeId: "challenge-1",
                legacySubmissionId: "10010001",
              }),
            }),
  ...
  test("apply-mode backfills existing submission file metadata on rerun", async () => {
    const tx = {
      challenge: {
        findMany: jest.fn().mockResolvedValue([]),
        create: jest.fn().mockResolvedValue({ id: "challenge-1" }),
    ...
    const submissionStore = {
      ...
      updateSubmissionMetadata: async ({ challengeId, legacySubmissionId }) => {
        const existingSubmission = submissionStoreRecords.get(legacySubmissionId);
        existingSubmission.systemFileName = buildSubmissionArchiveFileName({
          challengeId,
          legacySubmissionId,
        });
        existingSubmission.virusScan = true;
        existingSubmission.isFileSubmission = true;
      },
    };
    ...
    const result = await runApplyMode({
      ...
      options: {
        roundIds: ["9892"],
        skippedFilePath: path.join(fixtureDir, "apply-existing-submission-skipped.json"),
        importSubmissions: true,
        submissionStore,
        dataDir: fixtureDir,
        longComponentStateFile: "long_component_state_1.json",
    ...
    expect(submissionStoreRecords.get("10010001")).toEqual({
      legacySubmissionId: "10010001",
      memberId: "1",
      submitter: "alpha",
      systemFileName: buildSubmissionArchiveFileName({
        challengeId: "challenge-1",
        legacySubmissionId: "10010001",
      }),
      virusScan: true,
  ...
  test("apply-mode backfills existing submission archive URLs on rerun when archive generation is configured", async () => {
    const archiveDir = path.join(fixtureDir, "rerun-archives");
    fs.mkdirSync(archiveDir, { recursive: true });
    ...
    const submissionStore = {
      listExistingSubmissionsByLegacyId: async () => new Map(submissionStoreRecords),
      createSubmission: jest.fn(),
      updateSubmissionMetadata: jest.fn(),
    };
    const submissionArchiveStore = {
      listExistingSubmissionsByLegacyId: async () =>
        new Map([
          [
            ...
            {
              legacySubmissionId: "10010001",
              url: submissionStoreRecords.get("10010001").url,
            },
          ],
        ]),
      updateSubmissionUrl: jest.fn().mockImplementation(async ({ legacySubmissionId, url }) => {
    ...
    const result = await runApplyMode({
      ...
      options: {
        roundIds: ["9892"],
        skippedFilePath: path.join(fixtureDir, "apply-rerun-archive-submission-skipped.json"),
        importSubmissions: true,
        submissionStore,
        submissionArchiveStore,
        submissionArchiveDir: archiveDir,
    ...
    expect(submissionStoreRecords.get("10010001").url).toBe(
      buildSubmissionArchiveUrl({
        archiveFileName: buildSubmissionArchiveFileName({
          challengeId: "challenge-1",
          legacySubmissionId: "10010001",
